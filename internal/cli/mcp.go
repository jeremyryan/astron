...
import (
	...
	"github.com/project-astron/astron/internal/mcp"
)

// newMCPServerCmd builds the "mcp-server" command, which exposes the Astron read
// API as Model Context Protocol tools over stdio for local LLM agents.
func newMCPServerCmd(opts *options) *cobra.Command {
	var apiBaseURL string
	cmd := &cobra.Command{
		Use:   "mcp-server",
		Short: "Serve Astron retrieval as Model Context Protocol (MCP) tools over stdio",
		Long: "mcp-server exposes the Astron read API as Model Context Protocol (MCP)\n" +
			...
			"$ASTRON_API_URL. Logs go to stderr to keep stdout clean for the JSON-RPC\n" +
			"protocol.",
		Args: cobra.NoArgs,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "astron mcp-server: serving over stdio, API at %s\n", baseURL)
	...
	cmd.Flags().StringVar(&apiBaseURL, "api-base-url", "",
		"Base URL of the Astron read API (defaults to --server, then $ASTRON_API_URL)")
	return cmd
}

// resolveMCPBaseURL returns the read API base URL for the MCP server. It uses
// the global --server flag, falling back to $ASTRON_API_URL when --server is
...
func resolveMCPBaseURL(opts *options) string {
	if opts.server == defaultServer {
		if v := os.Getenv("ASTRON_API_URL"); v != "" {
			return v
		}
	}
	return opts.server
